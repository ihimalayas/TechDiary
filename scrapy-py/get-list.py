...
html_obj = session.get(f'file:///{pwd}/scrapy-py/test.html')
...
for i in bs.find_all('b'):
    if(len(i.text)>4 and len(i.text)<70):
        if(i.text in all_app_list):
            ...
            start_fail_num +=1
print("system has successfully started " + str(start_success_num) + "  Apps,  and Fail to startup  " + str(start_fail_num) + "  Apps!")
